...
<p>Currently COVID-19 testing can be done via a variety of molecular methods including: RT-qPCR, RT-PCR followed by standard PCR and gel electrophoresis, immunofluorescence microscopy, western blots for viral protein and even CT scans of infected lung tissue. COVID-19 is an +ssRNA virus, therefore analysis of its genetic material requires reverse-transcription of the RNA --> complementary DNA (cDNA). From a molecular diagnostic prospective the RT-qPCR method is the most definitively accurate and quick to perform. RT-qPCR is simply reverse transcription of the RNA into cDNA and then standard PCR amplification of the cDNA products at a specific known viral genome location.</p>

#### Ideas behind amplicon sequencing in COVID-19:
<p>Amplicon sequencing would take the RT-qPCR method a step further. Amplicon sequencing is simply target enriched sequencing. Instead of amplifying and quantifying one viral target in one patient per well (RT-qPCR), we would reverse transcribe and amplify multiple viral genome targets in each patient sample, then combine them into the same sample tube by patient and perform amplicon library assembly using specific barcodes for each patient sample. To make the process safe and fast we would need automated liquid handling robots to perform the heavy lifing/pipeting. After sequencing we can demultiplex those sample reads to each individual, align them to the viral genome and perform standard +/- virus analysis in addition to SNP calling and a myriad of other analyses.</p>  

#### Why amplicon sequencing instead of RT-qPCR:
I realized that using even the least capable next generation sequencer available (like an Illumina MiSeq, 15Gb w/25x10^6 reads) could not only drastically increase testing capacity for simple +/- diagnosis, but also increase its accuracy by using multiple viral amplicons. In addition we may be to learn about the COVID-19 virus as it spreads and evolves via SNP analysis of our amplicons, instead of just getting +/- answers. To do this we need to perform parallelized sequencing of multiple viral cDNAs in many patients simultaneously.</p>
